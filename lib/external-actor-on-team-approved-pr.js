...
async function main() {
    var _a;
    const http = new httpm.HttpClient("actions/external-actor-on-team-approved-pr");
    ...
    if (teamMemberLogins.includes(actor)) {
        core.debug(`Actor is a team member (${actor}), skipping Slack notification.`);
        return;
    }
    ...
    for (const review of reviews) {
        if (review.state === "APPROVED") {
            ...
            if (teamMemberLogins.includes(approverLogin)) {
                isApproved = true;
                core.debug(`The PR is approved by a team member: ${approverLogin}. Notifying Slack ...`);
                break;
            }
        }
    }
    ...
    // skip the Slack notification
    if (!isApproved) {
        core.debug("No team members have approved the PR yet, skipping Slack notification.");
        return;
    }
...
//# sourceMappingURL=external-actor-on-team-approved-pr.js.map